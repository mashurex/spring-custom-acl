...
/**
 * A customized {@link org.springframework.security.acls.domain.DefaultPermissionGrantingStrategy} to use
 * bitwise comparisons instead of direct equals comparison, allowing for hierarchical permissions.
 *
 * @author Mustafa Ashurex
 * @see org.springframework.security.acls.domain.DefaultPermissionGrantingStrategy
 */
		...
		for (Permission p : permission) {
			for (Sid sid : sids) {
				...
				for (AccessControlEntry ace : aces) {
					// Customized to use bitwise AND comparison so that 'higher' privileges can include
					// 'lesser' privileges. E.g. giving positive match when you only need a READ permission
					// but the user has ADMINISTRATION permission.
					int mask = ace.getPermission().getMask() & p.getMask();
					if (mask == p.getMask() && ace.getSid().equals(sid)) {
